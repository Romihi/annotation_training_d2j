...
- **推奨モデル**: `edgenext_small`、`mobilevit_s`、`vit_b_16`など
  ...
  - オートアノテーションの修正箇所が減少
- エポック数: 20-30程度
- **重要**: linearなど軽量モデルは推論は速いが精度が低いため、オートアノテーション用には非推奨
...
**オートアノテーション用推奨モデル（精度重視）**
- `edgenext_small`: 高精度で比較的軽量
- `mobilevit_s`: バランスが良く推奨
- `vit_b_16`: 最高精度（やや重い）
- `convnext_tiny`: 安定した高精度

**非推奨モデル（精度不足）**
- `linear`: シンプルすぎて複雑なコースに対応困難
- `cnn_simple`: 基本的なCNNで精度が低い
