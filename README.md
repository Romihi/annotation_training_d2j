...
3. **アノテーション作成**
   ...
   - **セグメンテーション**: 「B」キーで切替 → クリックでポリゴン頂点を配置
   - **ウェイポイント**: 「B」キーで切替 → クリックまたは一筆書きでウェイポイント配置
...
### 上部ツールバー
- **表示設定**: フォントサイズ、ダークモード切替、言語設定
- **クラウドメニュー**: Databricks / Google Colab連携
- **MLflow UI**: 実験追跡ダッシュボードへのアクセス

### 左パネル（設定・操作パネル）
- **データ読込エリア**: 画像フォルダ選択（複数フォルダ対応）、アノテーション読み込み
- **アノテーション設定**: クリック後自動スキップ、スキップ枚数設定
- **モード表示**: 現在のアノテーションモード（自動運転/物体検知/セグメンテーション/ウェイポイント）
- **走行モデル設定**: モデルアーキテクチャ選択、モデル読込、マルチモデル比較（最大4モデル同時）
- **推論設定**: 各モデルの推論表示ON/OFF、一括推論、GradCAM表示
- **YOLOモデル設定**: 物体検知・セグメンテーション用モデル選択

### 中央エリア（メインビューワー）
- **画像表示**: 現在選択中の画像とアノテーション結果
- **Speedバー**: 画像右側のインタラクティブな速度値表示・編集
...
| セグメンテーション | 「B」キー → クリックで頂点配置 | ポリゴン領域の描画 |
| ウェイポイント | 「B」キー → クリックまたは一筆書き | ウェイポイント配置 |
...
|------|------|
| `←` `→` | 前後の画像に移動（スキップ設定に連動） |
| `B` | アノテーションモード切替（自動運転→物体検知→セグメンテーション→ウェイポイント） |
| `Space` | 自動再生/停止（順方向） |
| `0-7` | 位置情報設定/解除 |
| `Delete` | アノテーション削除 |
...
1. 画像データセットの読み込み（複数フォルダ対応）
2. 運転アノテーション（ステアリング角度・スロットル・速度）の作成
3. 物体検知アノテーション（バウンディングボックス）の作成
4. セグメンテーションアノテーション（ポリゴン）の作成
5. ウェイポイントアノテーション（一筆書き・ガイドライン交点検出）の作成
6. 画像への位置情報の割り当て
7. モデルのトレーニングと評価（自動運転・YOLO・位置・ウェイポイント）
8. マルチモデル同時比較（最大4つの走行モデルを同時に推論・色分け表示）
9. 様々な形式でのアノテーションのエクスポート
...
- **セグメンテーション**: ポリゴンによる領域アノテーション（頂点編集・移動・リサイズ対応）
- **ウェイポイント**: 走行経路のウェイポイント配置（一筆書きモード・Y軸ガイドライン交点検出対応）
- **位置タグ付け**: 画像に位置識別子を割り当て
- **バッチ操作**: 自動アノテーション、範囲削除、ダウンサンプリングなど

### トレーニングと推論
- **モデルトレーニング**: 運転制御用のニューラルネットワークをトレーニング
- **マルチモデル比較**: 最大4つの走行モデルを同時に読み込み、推論結果を青系の異なる色で比較表示
  - メインモデル: シアン色
  - 追加モデル1: Royal Blue (`#4169E1`)
  - 追加モデル2: Dodger Blue (`#1E90FF`)
  - 追加モデル3: Medium Slate Blue (`#7B68EE`)
- **Speed出力**: 速度（speed）を追加出力として学習可能
- **将来予測出力**: 5フレーム先・10フレーム先のangle/throttle/speedを予測するモデルを学習可能
- **位置モデル**: 位置情報を予測する分類モデルの学習・推論
  - 複数画像入力: 複数カメラソースの同時入力（concat / アテンション融合）、または単一カメラからの仮想ソース生成（クロップ / スケール / 時間差スタック）
  - 出力の選択: 位置クラス分類、座標・姿勢回帰（pose / slam などの自己位置を教師データに使用、map座標系 [m] と方位）、格子分類（x, y を一辺 N m の格子に離散化してセルを分類）を任意に組み合わせ可能（マルチタスク）
  - 座標・姿勢回帰では、pose センサー（IMU デッドレコニング）が記録した車体姿勢角（roll, pitch）も出力に追加可能（自己位置ソースに slam 等を選んでいても roll/pitch は常に pose センサーの記録から取得）
  - 方位(θ)の教師データソースは、座標(x, y)と同じ自己位置ソース（既定）または pose センサーから選択可能（pose は再ローカライズのテレポートが無く連続的なため、座標は slam の絶対位置、方位は pose の滑らかな値、という組み合わせが可能）
  - 格子分類の推論は Top-N セルの確率を表示し、位置は Top1 セル中心または Top1〜N の確率重み付き平均から選択。マップ上には Top-N セルを濃淡で重ね描き
  - 時系列入力: 過去 N ステップの座標・姿勢 [x, y, θ] を画像特徴と併せて入力可能（学習時はノイズ付与・履歴欠損で画像依存を維持。推論時は実測または過去フレームの推論結果を履歴に使用）
  - LiDAR点群入力: 同セッションのLiDAR距離スキャン（lidar/*.npy）を軽量1D-CNNで符号化し画像特徴と併せて入力可能（間引き・正規化・無効値マスクはLiDAR Policyモデルと同じ前処理を再利用、積層フレーム数・間引きビン数・最大距離を設定可、学習時はスキャン全体をランダムに欠損させ画像依存を維持）
  - 推論時は学習時の入力構成をモデルから自動復元し、推定座標・姿勢と実測との誤差を情報パネルに表示
  - 「全画像を推論」で全フレームを推論し、走行軌跡マップに推定座標（紫の中空丸・現在フレームは紫三角＋誤差線、再生中も順次追加）と色分け「推論クラス」を重ねて確認可能
- **ウェイポイントモデル**: ウェイポイントを予測するモデルの学習・推論
- **YOLO統合**: 物体検知・セグメンテーション用のYOLOモデルのトレーニングと使用
- **データ拡張**: 拡張によるトレーニングデータの強化（プレビュー機能付き）
- **推論プレビュー**: 画像上でのモデル予測の表示（将来予測も含む）

### エクスポートオプション
- **Donkeycar形式**: Donkeycar用のアノテーションをエクスポート
- **Jetracer形式**: Jetracer用のアノテーションをエクスポート
- **YOLO形式**: YOLO用のアノテーションをエクスポート（検知・セグメンテーション両対応）
- **アノテーション動画**: アノテーションを可視化した動画の作成（フレームレート・ソース選択可能）

### UI・表示機能
- **ダークモード**: ツールバーからワンクリックで切替（設定は自動保存）
- **Speedバー**: 画像右側にインタラクティブな速度値バーを表示（クリックで速度値を設定、ホバーでプレビュー）
- **将来アノテーション表示**: t+5 / t+10フレーム先のアノテーション・推論結果を色分けで表示
- **ダウンサンプリング**: 直進区間など不要画像にダウンサンプリングマークを設定（青バッジで表示）

### 🏁 走行軌跡表示

自動運転アノテーションモードで、現在のステアリング角度に対応する予想走行軌跡を画像上にオレンジ色の弧でリアルタイム表示する機能です。アノテーション時の直感的な確認に役立ちます。

#### UIの場所と操作

左パネルの「走行軌跡を表示」チェックボックスをONにすると有効になります。以下のパラメータをスピンボックスで調整できます：

| パラメータ | デフォルト | 説明 |
|-----------|-----------|------|
| 最大舵角 | 25.0° | ステアリング値±1.0に対応する実際の切れ角（度） |
| 俯角 | 20.0° | カメラが水平から下を向いている角度。大きいほど軌跡が画像上部まで伸びる |
| FOV | 160.0° | カメラの対角視野角（度）。等距離射影モデルで使用 |

#### 地面座標系

車両を原点とする右手系の地面座標を使用します：

- **gx**：横方向（右が正）
- **gy**：前方距離（正が前）
- カメラ高さ **h** は等距離射影では比率としてのみ影響するため、実際の高さ値は不要

#### カメラ投影モデル（等距離射影魚眼）

FOV=160° の超広角レンズには等距離射影（Equidistant Fisheye）モデルを使用します：
...
ここで：
- `θ` = 光軸からの角度（rad）
- `f_fish = half_diagonal_pixels / (FOV_half_rad)` = 等距離射影焦点距離
- `r_image` = 主点からの画素距離

地面点 `(gx, gy)` からスクリーン座標への変換は以下の手順で行います：

```
# カメラ座標系（俯角α）
X_c = gx
Y_c = -gy·sin(α) + cos(α)    # 垂直成分
Z_c =  gy·cos(α) + sin(α)    # 奥行き成分

# 等距離射影
r_3d = √(X_c² + Y_c²)
θ = atan2(r_3d, Z_c)
r_img = f_fish × θ
ix = cx + r_img × (X_c / r_3d)
iy = cy + r_img × (Y_c / r_3d)
```

#### 円弧計算（地面座標系）

ステアリング角度 `angle_val`（-1.0 〜 +1.0）から円弧を計算します：

```python
steer_rad = radians(angle_val × max_steering)
gy_end = 4.0          # 前方4.0h先（カメラ高さを1とした正規化距離）
gx_end = gy_end × tan(steer_rad)

# 旋回半径（両方向共通の式）
R_g = (gx_end² + gy_end²) / (2 × gx_end)   # 右旋回: R_g > 0, 左旋回: R_g < 0

# パラメトリック円弧: t=0（車両直下）→ t=theta_total（終点）
gx(t) = R_g × (1 - cos t)
gy(t) = |R_g| × sin t
```

#### theta_total の導出（左右両対応）

終点の弧角 `theta_total` を求める際、`R_g` の符号によらず正しく動作する式を使用します：

```python
# 円弧の式をtについて解く
cos_t_end = (R_g - gx_end) / R_g      # cos t = (R_g - gx_end) / R_g
sin_t_end = gy_end / abs(R_g)          # sin t = gy_end / |R_g|  ← 常に正

theta_total = atan2(sin_t_end, cos_t_end)
```

- 右旋回（R_g > 0、gx_end > 0）：cos_t_end < 1、theta_total > 0 → 正しく右弧
- 左旋回（R_g < 0、gx_end < 0）：cos_t_end < 1、theta_total > 0 → 正しく左弧（gxがR_g*(1-cos t)により負になる）
- `atan2(gy_end, R_g - gx_end)` を直接使う方法はR_g < 0で誤った角度を返すため使用しない

#### クリッピングによる画像端への整合

弧の始点を車両直下（`gy_start ≈ 0`）に設定し、`painter.setClipRect(target_rect)` で画像領域外を切り落とすことで、弧が自然に画像下端中央から始まるように見せています：

```python
gy_start = gy_end * 0.005   # ほぼ0（車両直下）
painter.setClipRect(target_rect)   # 画像範囲外を切り抜き
# → 弧の車両近辺部分は画像外で見えず、画像下端から始まるように表示される
```

#### 160° 魚眼レンズの物理的解釈

4:3 画像（例：160×120px）での垂直方向半FOVは：

```
θ_vertical_half = atan(half_height / f_fish) ≈ 48°
```

俯角0°でも底辺中央はほぼ真下（地面）を向いており、**俯角0°でも地面は十分に見える**ため、`cam_height`（カメラ高さ）は独立したパラメータとして不要です（等距離射影では高さは投影比率に吸収されます）。
...
##### C. セグメンテーションアノテーション（ポリゴン）

1. **モードの切り替え**
   - 「Bキー」を押して「セグメンテーションモード」に切り替え
...
3. **ポリゴンの描画**
   - 画像上をクリックして頂点を配置
   - 頂点を順番にクリックしていくとポリゴンが形成されます
   - 最後の頂点をダブルクリックまたは最初の頂点付近をクリックして閉じる
...
1. **モードの切り替え**
   - 「Bキー」を押して「ウェイポイントモード」に切り替え

2. **ウェイポイントの配置**
   - **クリックモード**: 画像上をクリックしてウェイポイントを一つずつ配置
   - **一筆書きモード**: マウスボタンを押したままドラッグして連続的にウェイポイントを配置
   - **Y軸ガイドライン交点検出**: ガイドラインとの交点にウェイポイントを自動配置
...
2. **YOLO形式でエクスポート**（物体検知/セグメンテーションの場合）
   - 「YOLO」ボタンをクリック
...
1. **位置アノテーションの準備**
   - 右パネルの位置ボタン（0-7）を使って画像に位置タグを付与
   - コース上の各区間を識別できるように十分な画像にタグ付け
...
1. **ウェイポイントアノテーションの準備**
   - ウェイポイントモードで走行経路のポイントを配置

2. **学習の実行**
   - ウェイポイントモデル学習用のボタンから学習を開始
   - ウェイポイント座標を予測するモデルを学習

##### D. YOLOモデルの学習（物体検知・セグメンテーション）
...
2. **タスクタイプ選択**
   - 物体検知（detect）: バウンディングボックスによる物体検知
   - セグメンテーション（segment）: ポリゴンによる領域分割
...
4. **マルチモデル比較**
   - 左パネルの「＋ モデル追加」ボタンで追加モデルスロットを追加（最大3個、合計4モデル）
   - 各スロットでモデルアーキテクチャと保存モデルを個別に選択
   - 「モデル読込」ボタンで全モデルを一括読み込み
   - 各モデルの推論結果が異なる青系の色でキャンバス上に表示：
     - メインモデル: シアン色
     - 追加モデル1: Royal Blue
     - 追加モデル2: Dodger Blue
     - 追加モデル3: Medium Slate Blue
   - 各モデルの推論表示チェックボックスで個別に表示/非表示を制御
   - 「－ モデル削除」ボタンで追加スロットを後ろから削除可能

5. **バッチ推論**
   - 「全画像を推論」ボタンで一括推論（追加モデルも含めて一括実行）
   - 結果はCSVファイルにエクスポート可能
...
##### ダークモード
- 上部ツールバーの「ダークモード」ボタンで切り替え
- 設定は自動保存され、次回起動時にも適用

##### Speedバー
- 画像右側に速度値を表示するインタラクティブなバー
- クリックで速度値を設定、ホバーでプレビュー表示
- 推論結果のspeed値もシアン色の横線で表示
...
| B | アノテーションモード切替（自動運転→物体検知→セグメンテーション→ウェイポイント） |
| ← / → | 前/次の画像（自動スキップ設定に連動） |
| Delete / Backspace | 【自動運転モード】現在のアノテーション（angle/throttle/位置）を削除<br>【物体検知/セグメンテーション/ウェイポイントモード】選択項目の削除 |
| Space | 自動再生/停止 |
| S | 現在の設定を保存 |
| Ctrl + Z | 直前の操作を取り消し |
...
- `annotation/data_yolo/`: YOLOエクスポートファイル用
- `models/`: 保存されたトレーニング済みモデル（走行・位置・ウェイポイント）用
