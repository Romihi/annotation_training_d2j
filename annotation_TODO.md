...
d- ボタン表示一部修正
d- angle, throttleの統計情報（偏りがないか？をヒートマップ表現）を情報パネル下に表示

### 4/15
d- 位置選択を自動アノテーションチェックボックスで、次の画像にいったときも自動選択
d- 2画像対応
    - ２画像の連動読み出し
    - ２画像モデル構築

### 4/18
d- Jetson Orin Nano（JP6）対応でdocker版作成

###
- donkeycar lidar画像対応（画像の保存、統合を独立させて選択可能にする）
- donkeycar ２画像モデル対応
- mlflowのサーバー設定
- docker上でのmlflowアクセス設定
- リファクタリング
...
- ２画像モデルデータ取り、トライ
x- donkeyで変換したh5が動くか確認
→tensorflowのバージョンがdonkeycarの想定と合わなそう
