...
# 
# #CAMERA
CAMERA_TYPE = "LIDARIMAGE"   # (PICAM|WEBCAM|CVCAM|CSIC|V4L|D435|MOCK|IMAGE_LIST)
# LIDARIMAGE for CAM + LIDAR IMAGE
...
PWM_STEERING_THROTTLE = {
     ...
     "STEERING_LEFT_PWM": 310, #490, #480,               #pwm value for full left steering
     "STEERING_RIGHT_PWM": 450, #315, #330,              #pwm value for full right steering
     "THROTTLE_FORWARD_PWM": 450, #470           #pwm value for max forward throttle
     "THROTTLE_STOPPED_PWM": 390,            #pwm value for no movement
     "THROTTLE_REVERSE_PWM": 250 #250, #300,            #pwm value for max reverse throttle
}
#490
#320
...
DEFAULT_AI_FRAMEWORK = 'pytorch' #'tensorflow'
...
AI_THROTTLE_MULT = 1.0 #0.90              # this multiplier will scale every throttle value for all output from NN models
# 
# #Path following
...
HAVE_TMINI = True
if HAVE_TMINI:
     RECOVERY_DETECTION_DIV = 4 # N of detection div
     # 4つのゾーンに分割したLiDAR検出範囲を定義（前後左右）
     ZONE_INDEX = [[x for x in range(175,225)], 
                         [x for x in range(225,275)], 
                         [x for x in range(275,325)], 
                         [x for x in range(325,375)]]

# 壁検出のパラメータ
WALL_DETECTION_PARAMS = {
    'max_gap': 0.3,        # 点間の最大距離
    'min_wall_points': 8,  # 壁セグメントの最小点数
    'max_linearity': 0.08  # 最大許容直線偏差
}

# ライダー近傍の無視設定
IGNORE_DISTANCE = 0.15  # 単位:m、ライダー本体の周囲の部品の検出を除外する距離
...
HAVE_GS2 = False #True
if HAVE_GS2:
     RECOVERY_DETECTION_DIV = 3 # N of detection div
     ZONE_INDEX = [[x for x in range(30,80)], 
               [x for x in range(0,50)]+[x for x in range(130,160)], 
               [x for x in range(80,130)]]

#  # #RECOVERY SETTING
RECOVERY_DISTANCE = 120 # mm
RECOVERY_DETECTION_TIMES = 3 #N of detection times before recovering
RECOVERY_DURATION = 0.3 # seconds
RECOVERY_DURATION_BACK = 0.6 # seconds
...
NUM_LOGIC_ARRARY = 2
NUM_LOCIG_PIXEL = 10 * IMAGE_W
COLOR_FILTER = 'HSV' #HSV or RGB
LOWER_FILTER = [[60, 180, 180], [180, 180, 180]]
UPPER_FILTER = [[160, 255, 255], [280, 255, 255]]
ROI_CROP_TOP = 0 #45               # the number of rows of pixels to ignore on the top of the image
...
LAP_N = 4 # not deployed

### add originalflow senser
HAVE_OPTICALFLOW_PMW3901 =False

### add YOLO model from ultralytics
HAVE_YOLO = False
YOLO_SWITCH = False
YOLO_MODEL_PATH = "models/best_arrow_ncnn_model"
