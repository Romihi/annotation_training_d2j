...
環境変数を手で設定しなくても、アプリの **Cloud → Databricks → 設定** ダイアログで
すべての項目（Host / Token / Volumesパス / 実験パス / ノートブックパス / クラスター /
Catalog / Schema）を編集・保存できます。保存内容は
`databricks_settings.local.json`（gitignore対象）に永続化され、**アプリを再起動せずに**
反映されます。ダイアログからは次の操作が可能です。

- **接続テスト**: 入力したHost/Tokenで即座に接続確認
- **クラスター一覧を取得**: ワークスペースの全目的クラスターをドロップダウンから選択
  （自動学習に使うクラスターIDを手入力する必要がなくなります）
- **ノートブックを配置**: `databricks/` 配下の学習ノートブックをワークスペースの
  指定パスへ自動アップロード（手動インポート不要）

以降の環境変数の説明は、CI/ヘッドレス実行などアプリUIを使わない場合の代替手段です。
...
| `03_train_model.py` | 基本モデル（ステア/スロットル回帰・resnet18）の学習 |
| `04_train_togivad.py` | TogiVAD（軌道語彙分類）の学習。`togivad/future_traj` ラベルが必要 |

> **アプリからのワンクリック学習**: **Cloud → Databricks → 転送** で「転送後に自動学習」を
> ONにすると、ノートブックの自動配置 → データ転送 → 学習ジョブ送信 → 進捗監視 →
> 学習済みモデルのローカル取り込み、までをアプリ内で一気通貫に実行できます。
> モデル種別（基本 / TogiVAD）とハイパラ（エポック数・バッチ・学習率・語彙数K）も
> ダイアログから指定できます。TogiVAD学習には、マップビューの
> 「軌道ラベルを計算して保存」で `togivad/future_traj` を付与したデータが必要です。
