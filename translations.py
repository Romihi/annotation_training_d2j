...
TRANSLATIONS = {
    ...
    'ja': {
        ...
        'label_attention_weights': 'カメラ貢献度 (Attention)',
        ...
        'label_model_architecture': 'モデルタイプ:',
        ...
        # ドメインランダマイゼーション
        'group_domain_randomization': 'ドメインランダマイゼーション',
        'chk_aug_hue': '色相シフト (A)',
        'label_hue_range': '色相範囲 (±):',
        'label_hue_prob': '確率:',
        'chk_aug_grayscale': 'グレースケール化 (A)',
        'label_grayscale_prob': '確率:',
        'chk_aug_blur': 'ガウシアンブラー (B)',
        'label_blur_kernel': 'カーネルサイズ:',
        'label_blur_prob': '確率:',
        'chk_aug_motion_blur': 'モーションブラー (B)',
        'label_motion_kernel': 'カーネルサイズ:',
        'label_motion_prob': '確率:',
        'chk_aug_fog': '霧・霞 (D)',
        'label_fog_intensity': '霧の濃度:',
        'label_fog_prob': '確率:',
        'chk_aug_noise': 'ガウシアンノイズ (A)',
        'label_noise_std': 'ノイズ強度 (std):',
        'label_noise_prob': '確率:',
        # 強光・路面グレア
        'group_sunlight': '強光・路面グレア',
        'chk_aug_sunspot': 'サンスポット（路面グレア）',
        'label_sunspot_intensity': 'グレア強度:',
        'label_sunspot_num': 'スポット数:',
        'label_sunspot_prob': '確率:',
        'chk_aug_gamma': 'ガンマ変動（露出シミュレーション）',
        'label_gamma_min': 'γ 最小:',
        'label_gamma_max': 'γ 最大:',
        'label_gamma_prob': '確率:',
        'chk_aug_shadow': '影コントラスト（日向/日陰境界）',
        'label_shadow_darkness': '影の暗さ:',
        'label_shadow_prob': '確率:',
        ...
        'label_training_image_sources': '入力画像ソース',
        'label_training_image_sources_info': '学習に使用する画像ソースを選択（複数選択で学習データが増加）',
        'label_current_source_marker': '{0} ({1}枚) [現在表示中]',
        'label_other_source_marker': '{0} ({1}枚)',
        'label_training_sources_count': '選択中: {0}ソース / 推定学習画像数: {1}枚',
        'label_training_sources_count_multi': '選択中: {0}ソース / 推定サンプル数: {1}件 ({2}画像/サンプル)',
        'label_fusion_settings': '特徴融合設定（マルチソース入力）',
        'label_fusion_info': '複数ソースの画像を同時入力してモデルに学習させます',
        'label_fusion_method': '融合方法:',
        'fusion_concat': '特徴結合 (concat) - 各画像の特徴量を結合',
        'fusion_attention': 'アテンション融合 - 画像間の関連性を学習',
        'label_multi_source_note': '※ マルチソースモデルは推論時にも全ソースの画像が必要です',
        'label_virtual_source_settings': '仮想ソース生成（単一カメラ）',
        'label_virtual_source_type': '生成方式:',
        'opt_virtual_none': 'なし（通常）',
        'opt_virtual_crop': 'A: 空間クロップ（左/中/右）',
        'opt_virtual_scale': 'B: スケールピラミッド（全体/2x/4x zoom）',
        'opt_virtual_temporal': 'C: 時間差スタック',
        'label_virtual_num_sources': '仮想ソース数:',
        'label_temporal_interval': '時間差(フレーム):',
        'label_virtual_source_note': '※ 単一カメラから仮想的に複数ソースを生成します',
        'label_resolution_scale': '解像度:',
        'tooltip_resolution_scale': '学習・表示解像度（100%=フル解像度 / 低いほどブロック状に）',
        'label_resolution_mode_group': '解像度モード設定',
        'label_resolution_mode_current': 'メインスライダー設定: {0}%',
        'opt_resolution_pixelate': 'ピクセレーション（入力サイズ維持・画像内容を劣化）',
        'opt_resolution_resize': 'サイズ縮小（入力解像度を実際に下げてアーキテクチャを変更）',
        'tip_virtual_overlay': '仮想ソース生成方式をキャンバスに可視化\n学習ダイアログで同じ仮想ソース生成方式を選択すると、このプレビューと同じ入力形式で学習できます',
        'opt_virtual_overlay_none': '仮想ソース描画',
        'opt_virtual_overlay_crop': '空間クロップ',
        'opt_virtual_overlay_scale': 'スケールピラミッド',
        'opt_virtual_overlay_temporal': '時間差スタック',
        'label_multi_source_model_marker': '[{0}src:{1}]',
        'dlg_multi_source_select': 'マルチソースモデル - ソース選択',
        'label_multi_source_model_info': 'このモデルは{0}個の画像ソースを使用します（融合: {1}）',
        'label_multi_source_select_sources': '推論に使用する画像ソースを選択してください:',
        'label_multi_source_trained_sources': '学習時のソース: {0}',
        'msg_multi_source_need_sources': '画像ソースを{0}個選択してください（現在: {1}個）',
        'label_multi_source_link_combined': '選択したソースで結合表示に切り替え',
        'status_multi_source_model_loaded': 'マルチソースモデルを読み込みました（{0}ソース, {1}融合）',
        'label_training_params': '学習パラメータ',
        'label_speed_data_info': '※ {0}個のアノテーションにspeedデータが含まれています',
        'tip_speed_normalize': 'Speed値(m/s)を正規化する際の除数（デフォルト: config.MAX_SPEED）',
        ...
        'tip_auto_driving_direction': 'アノテーション点のangle値から操舵軌道（予測円弧）を表示',
        'tip_recorded_trajectory': '記録された自己位置データから計算した実際の走行軌道を緑色で画像上に表示',
        'placeholder_recorded_traj_points': '20',
        ...
        'tip_auto_cam_height': 'カメラの地上高（m）。走行軌道（実測）の投影スケールに使用',
        'label_auto_cam_pitch': 'カメラ俯角:',
        'label_auto_fov': '画角FOV:',
        'label_auto_cam_height': 'カメラ高:',
        ...
        'label_bin_avg': '区間平均',
        'label_normalized': '正規化',
        'label_normalized_title': 'センサー値（各最大値で正規化）',
        'label_window': '窓:',
        'label_bin': '区間:',
        'label_display_range': '表示範囲:',
        'label_auto': '自動',
        'label_display_items': '表示項目:',
        'label_click_to_jump': 'クリック: ジャンプ',
        'label_zoom_hint': 'ホイール: 拡大/縮小　右ドラッグ: 移動',
        'btn_zoom_reset': 'ズームリセット',
        'btn_close': '閉じる',
...
================================================================================''',
        ...
        # 時系列モデル関連
        'btn_traj_train': '学習・保存',
        'dlg_traj_training_settings': '時系列モデル - 学習設定',
        'label_image_sources': '画像ソース選択',
        'label_sources_selected': '{0} ソース選択中',
        'label_sources_max_exceeded': '最大{0}ソースまで選択可能です',
        'msg_select_at_least_one_source': '画像ソースを1つ以上選択してください',
        'label_model_arch': 'モデルタイプ',
        'label_arch_params': 'モデルタイプ パラメータ',
        'label_seq_len': '入力シーケンス長',
        'label_pred_horizon': '予測ステップ数',
        'label_togivad_params': 'TogiVAD パラメータ',
        'label_pose_source': '軌道ラベルの自己位置ソース',
        'opt_pose_source_pose': 'pose（デッドレコニング・既定）',
        'opt_pose_source_slam': 'slam（LiDAR SLAM）',
        'label_togivad_pred_seconds': '予測秒数',
        'label_togivad_pred_points': '予測点数',
        ...
        'label_togivad_info': ('軌道: 1.0秒 / 20点（MPPI互換）。画像ソース数は '
                               ...
                               '（テレポート等）は自動除外されます'),
        'msg_togivad_bad_source_count': ('TogiVADの画像ソース数は 1/2/4/5 の'
                                         'いずれかを選択してください'),
        'msg_togivad_no_pose': ('自己位置データ（pose/slam）がありません。'
        ...
        'label_fusion_method': '融合方法',
        'label_attn_heads': 'Attn ヘッド数',
        'opt_fusion_concat': '連結（Concat）',
        'opt_fusion_attention': 'アテンション（Attention）',
        'label_tcn_channels': 'TCN チャネル数',
        'label_tcn_kernel_size': 'TCN カーネルサイズ',
        'label_cnn_channels': 'CNN チャネル数',
        'label_cnn_kernel_size': 'CNN カーネルサイズ',
        'msg_traj_training': '時系列モデルの学習中...',
        'dlg_traj_model_training': '時系列モデル学習',
        'msg_traj_training_complete': '時系列モデルの学習が完了しました',
        'msg_traj_no_sequences': '有効なシーケンスが生成されませんでした。データ範囲またはシーケンス長を確認してください。',
        ...
        'msg_traj_prediction_complete': '時系列モデルの推論が完了しました',
        'msg_traj_prediction_count': '{0} フレームの予測を生成しました',
        'msg_no_traj_models': '時系列モデルが見つかりません。先に学習を実行してください。',
        ...
        # 後方互換 (GRUキー)
        'btn_gru_train': '学習・保存',
        'btn_gru_predict': 'モデル読込',
        'dlg_gru_training_settings': '時系列モデル - 学習設定',
        'label_gru_params': 'アーキテクチャ パラメータ',
        'label_gru_hidden': 'Hidden Size',
        'msg_gru_training': '時系列モデルの学習中...',
        'dlg_gru_model_training': '時系列モデル学習',
        'msg_gru_training_complete': '時系列モデルの学習が完了しました',
        'msg_gru_no_sequences': '有効なシーケンスが生成されませんでした。データ範囲またはシーケンス長を確認してください。',
        'label_gru_seq_info': '入力: {0}フレーム → 予測: {1}フレーム',
        'dlg_gru_predict_settings': '時系列モデル - 推論設定',
        'dlg_gru_prediction': '時系列モデル',
        'msg_gru_predicting': '時系列モデルの推論中...',
        'msg_gru_prediction_complete': '時系列モデルの推論が完了しました',
        'msg_gru_prediction_count': '{0} フレームの予測を生成しました',
        'msg_no_gru_models': '時系列モデルが見つかりません。先に学習を実行してください。',
        'label_select_gru_model': '時系列モデル選択',
        'label_gru_model_info': 'seq_len={0}, pred_horizon={1}, sources={2}',
        'chk_show_gru_prediction': '時系列予測軌道を表示',
        'label_gru_pred_steering': '予測(steering)',
        'label_gru_pred_throttle': '予測(throttle)',
        'label_gru_section_title': '時系列モデル',
        ...
        'map_view_load_background': '地図を読込',
        ...
        'map_view_quality_threshold_label': 'ジャンプ閾値[m]:',
        'map_view_quality_detect_btn': '検出',
        'map_view_quality_mark_deleted_btn': '削除としてマーク',
        'map_view_quality_result': '{0}件のフレームを検出しました（ジャンプ/status異常）',
        'map_view_quality_none': '品質問題は検出されませんでした',
        'map_view_quality_confirm_title': '品質フィルタ確認',
        'map_view_quality_confirm_msg': '{0}件のフレームを削除としてマークします。よろしいですか？',
        'map_view_quality_no_data': '先に品質フィルタで検出を実行してください',

        'map_view_segment_label': '区間編集:',
        'map_view_segment_start_label': '開始:',
        'map_view_segment_end_label': '終了:',
        'map_view_segment_source_label': 'ソース:',
        'map_view_segment_apply_btn': '適用',
        'map_view_segment_clear_btn': '上書きをクリア',

        'map_view_interp_label': '欠損補間:',
        'map_view_interp_maxgap_label': '最大ギャップ:',
        'map_view_interp_run_btn': '補間実行',
        'map_view_interp_clear_btn': '補間をクリア',
        'map_view_interp_result': '{0}件のフレームを補間しました',

        'map_view_writeback_label': '学習用軌道ラベル:',
        ...
        'map_view_writeback_dt_tooltip': '将来軌道の点の時間間隔（デフォルト0.05秒 = togivadの設定と同じ。20点×0.05秒=1秒先まで）',
        'map_view_writeback_confirm_title': '学習用軌道ラベルの保存',
        'map_view_writeback_confirm_msg': '全{0}フレームの将来軌道（togivad学習用の教師ラベル）を計算し、\n{1}個のcatalogファイルに保存します。\n\n・元のファイルは .bak としてバックアップされます\n・計算中はキャンセルできます（キャンセル時はファイルを変更しません）\n\n実行しますか？',
        'map_view_writeback_progress': '将来軌道を計算中... {0}/{1} フレーム',
        'map_view_writeback_saving': 'catalogファイルへ保存中...',
        'map_view_writeback_cancelled': 'キャンセルしました。ファイルは変更されていません。',
        ...
        'map_view_tab_pose_edit': '自己位置の補正',
        'map_view_tab_labels': '学習ラベル',
        'map_view_tab_regions': '位置領域',
        ...
        'db_group_cluster': 'クラスター（自動学習用）',
        'db_btn_refresh_clusters': 'クラスター一覧を取得',
        'db_btn_save': '保存',
        'db_btn_test': '接続テスト',
        'db_btn_deploy': 'ノートブックを配置',
        'db_tip_deploy': 'アプリ同梱の学習ノートブックをワークスペースの指定パスへ配置（上書き）します。',
        'db_msg_saved': '設定を保存しました（再起動不要で反映されます）。',
        'db_msg_testing': '接続をテスト中...',
        'db_msg_test_failed': '接続テスト失敗: {0}',
        'db_msg_loading_clusters': 'クラスター一覧を取得中...',
        'db_msg_clusters_loaded': '{0}個のクラスターを取得しました。',
        'db_msg_clusters_failed': 'クラスター一覧の取得に失敗: {0}',
        'db_msg_deploying': 'ノートブックを配置中...',
        'db_msg_deployed': '{0}個のノートブックを配置しました: {1}',
        'db_msg_deploy_failed': 'ノートブックの配置に失敗: {0}',
        'db_train_settings': '学習設定（自動学習ON時）',
        ...
        'db_field_model_type': 'モデル種別:',
        'db_field_epochs': 'エポック数:',
        'db_field_batch': 'バッチサイズ:',
        'db_field_lr': '学習率:',
        'db_field_vocab_k': '語彙数 K:',
        'db_monitor_title': 'Databricks学習ジョブの進捗',
        'db_monitor_starting': 'ジョブの状態を取得中...',
        'db_monitor_hint': 'このウィンドウは開いたまま他の操作ができます。閉じてもDatabricks側の学習は継続します（進捗はブラウザで確認可）。開いたままにすると、完了時に学習済みモデルの取り込みを案内します。',
        'db_monitor_open_browser': 'ブラウザで開く',
        'db_monitor_status': 'Run {0}: {1} {2}',
        'db_monitor_poll_error': '状態の取得中にエラー: {0}',
        'db_monitor_success': 'Run {0}: 学習が正常に完了しました。',
        'db_monitor_failed': 'Run {0}: 失敗 ({1}) {2}',
        'db_menu_import_model': 'モデルを取り込み',
        'db_import_title': '学習済みモデルの取り込み',
        'db_import_desc': 'Databricksの models/ から取り込むモデルファイルを選択してください（新しい順）。',
        'db_import_refresh': '一覧を更新',
        'db_import_list_failed': 'モデル一覧の取得に失敗: {0}',
        'db_import_overwrite': '同名のファイルが既に存在します。上書きしますか？\n{0}',
        'db_dl_title': '学習済みモデルの取り込み',
        'db_dl_confirm': '学習済みモデルをローカルの models/ に取り込みますか？',
        'db_dl_none': 'Volumesのmodels/に取り込めるモデルファイル（.pt/.pth）が見つかりませんでした。',
        'db_dl_progress': 'モデルをダウンロード中...',
        'db_dl_done': 'モデルを取り込みました:\n{0}',
        'db_dl_failed': 'モデルの取り込みに失敗: {0}',
        'db_menu_pull_records': 'クラウド記録を取得',
        'db_pull_progress': 'クラウドの学習記録を取得中...',
        'db_pull_done': 'クラウド記録の取得が完了しました。\n取得: {0}件 / 既存スキップ: {1}件 / 失敗: {2}件',
        'db_pull_cancelled': '取得をキャンセルしました。',
        'db_pull_failed': 'クラウド記録の取得に失敗しました:\n{0}',
        'db_menu_logs': 'ジョブログ取得',
        'db_logs_title': '学習パイプラインのログ',
        'db_logs_select_run': '対象のRun:',
        'db_logs_fetch': 'ログ取得',
        'db_logs_fetching': 'ログを取得中...',
        'db_logs_fetch_failed': 'ログの取得に失敗しました: {0}',
        'db_logs_save': 'ファイルに保存',
        'db_logs_saved': 'ログを保存しました:\n{0}',
        'db_logs_save_failed': 'ログの保存に失敗: {0}',
    },
    ...
    'en': {
        ...
        'label_data_distribution': 'Data Analysis',
        ...
        'label_attention_weights': 'Camera Attention',
        ...
        'label_model_architecture': 'Model Type:',
        ...
        'group_domain_randomization': 'Domain Randomization',
        'chk_aug_hue': 'Hue Shift (A)',
        'label_hue_range': 'Hue Range (±):',
        'label_hue_prob': 'Prob:',
        'chk_aug_grayscale': 'Grayscale (A)',
        'label_grayscale_prob': 'Probability:',
        'chk_aug_blur': 'Gaussian Blur (B)',
        'label_blur_kernel': 'Kernel Size:',
        'label_blur_prob': 'Prob:',
        'chk_aug_motion_blur': 'Motion Blur (B)',
        'label_motion_kernel': 'Kernel Size:',
        'label_motion_prob': 'Prob:',
        'chk_aug_fog': 'Fog / Haze (D)',
        'label_fog_intensity': 'Fog Intensity:',
        'label_fog_prob': 'Prob:',
        'chk_aug_noise': 'Gaussian Noise (A)',
        'label_noise_std': 'Noise Std:',
        'label_noise_prob': 'Prob:',
        # Sunlight
        'group_sunlight': 'Sunlight / Road Glare',
        'chk_aug_sunspot': 'Sunspot (Road Glare)',
        'label_sunspot_intensity': 'Glare Intensity:',
        'label_sunspot_num': 'Num Spots:',
        'label_sunspot_prob': 'Prob:',
        'chk_aug_gamma': 'Gamma Variation (Exposure Sim.)',
        'label_gamma_min': 'γ Min:',
        'label_gamma_max': 'γ Max:',
        'label_gamma_prob': 'Prob:',
        'chk_aug_shadow': 'Shadow Contrast (Sun/Shade Border)',
        'label_shadow_darkness': 'Shadow Darkness:',
        'label_shadow_prob': 'Prob:',
        ...
        'label_training_image_sources': 'Input Image Sources',
        'label_training_image_sources_info': 'Select image sources for training (multiple selections increase training data)',
        'label_current_source_marker': '{0} ({1} images) [currently displayed]',
        'label_other_source_marker': '{0} ({1} images)',
        'label_training_sources_count': 'Selected: {0} source(s) / Estimated training images: {1}',
        'label_training_sources_count_multi': 'Selected: {0} source(s) / Estimated samples: {1} ({2} images/sample)',
        'label_fusion_settings': 'Feature Fusion Settings (Multi-Source Input)',
        'label_fusion_info': 'Train the model with simultaneous multi-source image input',
        'label_fusion_method': 'Fusion Method:',
        'fusion_concat': 'Feature Concatenation (concat) - Concatenate features from each image',
        'fusion_attention': 'Attention Fusion - Learn inter-image relationships',
        'label_multi_source_note': '* Multi-source models require all source images during inference',
        'label_virtual_source_settings': 'Virtual Source Generation (Single Camera)',
        'label_virtual_source_type': 'Generation Mode:',
        'opt_virtual_none': 'None (Standard)',
        'opt_virtual_crop': 'A: Spatial Crops (Left/Center/Right)',
        'opt_virtual_scale': 'B: Scale Pyramid (Full/2x/4x zoom)',
        'opt_virtual_temporal': 'C: Temporal Stack',
        'label_virtual_num_sources': 'Virtual Sources:',
        'label_temporal_interval': 'Frame Interval:',
        'label_virtual_source_note': '* Generates multiple virtual sources from a single camera image',
        'label_resolution_scale': 'Resolution:',
        'tooltip_resolution_scale': 'Training/display resolution (100%=full / lower=pixelated)',
        'label_resolution_mode_group': 'Resolution Mode',
        'label_resolution_mode_current': 'Main slider: {0}%',
        'opt_resolution_pixelate': 'Pixelation (keep input size, degrade content)',
        'opt_resolution_resize': 'Size reduction (actually reduce input resolution)',
        'tip_virtual_overlay': 'Visualize virtual source generation on canvas\nSelect the same type in the training dialog to train with this input format',
        'opt_virtual_overlay_none': 'Virtual Source View',
        'opt_virtual_overlay_crop': 'Spatial Crop',
        'opt_virtual_overlay_scale': 'Scale Pyramid',
        'opt_virtual_overlay_temporal': 'Temporal Stack',
        'label_multi_source_model_marker': '[{0}src:{1}]',
        'dlg_multi_source_select': 'Multi-Source Model - Source Selection',
        'label_multi_source_model_info': 'This model uses {0} image sources (fusion: {1})',
        'label_multi_source_select_sources': 'Select image sources for inference:',
        'label_multi_source_trained_sources': 'Trained sources: {0}',
        'msg_multi_source_need_sources': 'Please select {0} sources (current: {1})',
        'label_multi_source_link_combined': 'Switch to combined view with selected sources',
        'status_multi_source_model_loaded': 'Multi-source model loaded ({0} sources, {1} fusion)',
        'label_training_params': 'Training Parameters',
        'label_speed_data_info': '* {0} annotations contain speed data',
        'tip_speed_normalize': 'Divisor for normalizing speed values in m/s (default: config.MAX_SPEED)',
        ...
        'tip_auto_driving_direction': 'Display steering trajectory (predicted arc) from annotation angle value',
        'tip_recorded_trajectory': 'Overlay the actual driven trajectory (computed from recorded pose data) in green',
        'placeholder_recorded_traj_points': '20',
        ...
        'tip_auto_cam_height': 'Camera height above ground (m). Used to scale the driven-trajectory projection',
        'label_auto_cam_pitch': 'Cam Pitch:',
        'label_auto_fov': 'FOV:',
        'label_auto_cam_height': 'Cam Height:',
        ...
        'label_normalized_title': 'Sensor Values (normalized by max)',
        'label_window': 'Window:',
        'label_bin': 'Bin:',
        'label_display_range': 'Range:',
        'label_auto': 'Auto',
        'label_display_items': 'Display Items:',
        'label_click_to_jump': 'Click: Jump',
        'label_zoom_hint': 'Wheel: Zoom  Right-drag: Pan',
...
================================================================================''',
        ...
        'btn_traj_train': 'Train',
        'dlg_traj_training_settings': 'Time-Series Sequence Model - Training Settings',
        'label_image_sources': 'Image Source Selection',
        'label_sources_selected': '{0} source(s) selected',
        'label_sources_max_exceeded': 'Maximum {0} sources allowed',
        'msg_select_at_least_one_source': 'Please select at least 1 image source',
        'label_model_arch': 'Model Type',
        'label_arch_params': 'Model Type Parameters',
        'label_seq_len': 'Input Sequence Length',
        'label_pred_horizon': 'Prediction Horizon',
        'label_togivad_params': 'TogiVAD Parameters',
        'label_pose_source': 'Localization source for trajectory labels',
        'opt_pose_source_pose': 'pose (dead reckoning, default)',
        'opt_pose_source_slam': 'slam (LiDAR SLAM)',
        'label_togivad_pred_seconds': 'Predict seconds',
        'label_togivad_pred_points': 'Predict points',
        ...
        'label_togivad_info': ('Trajectory: 1.0s / 20 points (MPPI compatible). '
                               ...
                               'automatically'),
        'msg_togivad_bad_source_count': ('TogiVAD requires 1, 2, 4 or 5 '
                                         'image sources'),
        'msg_togivad_no_pose': ('No localization data (pose/slam) found. '
                                'TogiVAD training requires recordings with '
                                'localization'),
        ...
        'label_hidden_dim': 'Hidden Size',
        'label_dropout': 'Dropout',
        'label_gru_layers': 'GRU Layers',
        'label_fusion_method': 'Fusion Method',
        'label_attn_heads': 'Attn Heads',
        'opt_fusion_concat': 'Concatenation',
        'opt_fusion_attention': 'Attention',
        'label_tcn_channels': 'TCN Channels',
        'label_tcn_kernel_size': 'TCN Kernel Size',
        'label_cnn_channels': 'CNN Channels',
        'label_cnn_kernel_size': 'CNN Kernel Size',
        'msg_traj_training': 'Training time-series sequence model...',
        'dlg_traj_model_training': 'Time-Series Sequence Model Training',
        'msg_traj_training_complete': 'Time-series sequence model training completed',
        'msg_traj_no_sequences': 'No valid sequences generated. Check data range or sequence length.',
        ...
        'msg_traj_prediction_complete': 'Time-series sequence inference completed',
        'msg_traj_prediction_count': 'Generated predictions for {0} frames',
        'msg_no_traj_models': 'No sequence models found. Please run training first.',
        ...
        # Legacy GRU keys
        'btn_gru_train': 'Train',
        'btn_gru_predict': 'Run Inference',
        'dlg_gru_training_settings': 'Time-Series Sequence Model - Training Settings',
        'label_gru_params': 'Architecture Parameters',
        'label_gru_hidden': 'Hidden Size',
        'msg_gru_training': 'Training time-series sequence model...',
        'dlg_gru_model_training': 'Time-Series Sequence Model Training',
        'msg_gru_training_complete': 'Time-series sequence model training completed',
        'msg_gru_no_sequences': 'No valid sequences generated. Check data range or sequence length.',
        'label_gru_seq_info': 'Input: {0} frames → Predict: {1} frames',
        'dlg_gru_predict_settings': 'Time-Series Sequence - Inference Settings',
        'dlg_gru_prediction': 'Time-Series Sequence Prediction',
        'msg_gru_predicting': 'Running time-series sequence inference...',
        'msg_gru_prediction_complete': 'Time-series sequence inference completed',
        'msg_gru_prediction_count': 'Generated predictions for {0} frames',
        'msg_no_gru_models': 'No sequence models found. Please run sequence training first.',
        'label_select_gru_model': 'Select Sequence Model',
        'label_gru_model_info': 'seq_len={0}, pred_horizon={1}, sources={2}',
        'chk_show_gru_prediction': 'Show Predicted Sequence',
        'label_gru_pred_steering': 'Pred (steering)',
        'label_gru_pred_throttle': 'Pred (throttle)',
        'label_gru_section_title': 'Time-Series Sequence Model',
        ...
        'map_view_load_background': 'Load Map',
        ...
        'map_view_quality_threshold_label': 'Jump threshold [m]:',
        'map_view_quality_detect_btn': 'Detect',
        'map_view_quality_mark_deleted_btn': 'Mark as deleted',
        'map_view_quality_result': 'Detected {0} frame(s) (jump / bad status)',
        'map_view_quality_none': 'No quality issues detected',
        'map_view_quality_confirm_title': 'Confirm quality filter',
        'map_view_quality_confirm_msg': 'Mark {0} frame(s) as deleted. Continue?',
        'map_view_quality_no_data': 'Run detection first',

        'map_view_segment_label': 'Segment edit:',
        'map_view_segment_start_label': 'Start:',
        'map_view_segment_end_label': 'End:',
        'map_view_segment_source_label': 'Source:',
        'map_view_segment_apply_btn': 'Apply',
        'map_view_segment_clear_btn': 'Clear overrides',

        'map_view_interp_label': 'Gap interpolation:',
        'map_view_interp_maxgap_label': 'Max gap:',
        'map_view_interp_run_btn': 'Interpolate',
        'map_view_interp_clear_btn': 'Clear interpolation',
        'map_view_interp_result': 'Interpolated {0} frame(s)',

        'map_view_writeback_label': 'Training trajectory labels:',
        ...
        'map_view_writeback_dt_tooltip': 'Time interval between trajectory points (default 0.05 s = togivad default; 20 pts × 0.05 s = 1 s ahead)',
        'map_view_writeback_confirm_title': 'Save training trajectory labels',
        'map_view_writeback_confirm_msg': 'Compute future trajectories (togivad training labels) for all {0} frames\nand save them into {1} catalog file(s).\n\n- Original files are backed up as .bak\n- You can cancel during computation (no files are changed on cancel)\n\nProceed?',
        'map_view_writeback_progress': 'Computing future trajectories... {0}/{1} frames',
        'map_view_writeback_saving': 'Saving to catalog files...',
        'map_view_writeback_cancelled': 'Cancelled. No files were changed.',
        ...
        'map_view_tab_pose_edit': 'Pose correction',
        'map_view_tab_labels': 'Training labels',
        'map_view_tab_regions': 'Location regions',
        ...
        'db_group_cluster': 'Cluster (for auto-training)',
        'db_btn_refresh_clusters': 'Fetch clusters',
        'db_btn_save': 'Save',
        'db_btn_test': 'Test connection',
        'db_btn_deploy': 'Deploy notebooks',
        'db_tip_deploy': 'Deploys the bundled training notebooks to the specified workspace path (overwrite).',
        'db_msg_saved': 'Settings saved (applied without restart).',
        'db_msg_testing': 'Testing connection...',
        'db_msg_test_failed': 'Connection test failed: {0}',
        'db_msg_loading_clusters': 'Fetching clusters...',
        'db_msg_clusters_loaded': 'Fetched {0} cluster(s).',
        'db_msg_clusters_failed': 'Failed to fetch clusters: {0}',
        'db_msg_deploying': 'Deploying notebooks...',
        'db_msg_deployed': 'Deployed {0} notebook(s): {1}',
        'db_msg_deploy_failed': 'Failed to deploy notebooks: {0}',
        'db_train_settings': 'Training settings (when auto-train is on)',
        ...
        'db_field_model_type': 'Model type:',
        'db_field_epochs': 'Epochs:',
        'db_field_batch': 'Batch size:',
        'db_field_lr': 'Learning rate:',
        'db_field_vocab_k': 'Vocabulary K:',
        'db_monitor_title': 'Databricks training job progress',
        'db_monitor_starting': 'Fetching job status...',
        'db_monitor_hint': 'You can leave this window open and keep using the app. Closing it does not stop the training on Databricks (check progress in the browser). Keep it open to be prompted to import the trained model when it finishes.',
        'db_monitor_open_browser': 'Open in browser',
        'db_monitor_status': 'Run {0}: {1} {2}',
        'db_monitor_poll_error': 'Error while fetching status: {0}',
        'db_monitor_success': 'Run {0}: training completed successfully.',
        'db_monitor_failed': 'Run {0}: failed ({1}) {2}',
        'db_menu_import_model': 'Import model',
        'db_import_title': 'Import trained model',
        'db_import_desc': 'Select a model file to import from Databricks models/ (newest first).',
        'db_import_refresh': 'Refresh list',
        'db_import_list_failed': 'Failed to fetch model list: {0}',
        'db_import_overwrite': 'A file with the same name already exists. Overwrite?\n{0}',
        'db_dl_title': 'Import trained model',
        'db_dl_confirm': 'Import the trained model into your local models/ folder?',
        'db_dl_none': 'No importable model file (.pt/.pth) was found under Volumes models/.',
        'db_dl_progress': 'Downloading model...',
        'db_dl_done': 'Model imported:\n{0}',
        'db_dl_failed': 'Failed to import model: {0}',
        'db_menu_pull_records': 'Pull cloud records',
        'db_pull_progress': 'Pulling cloud training records...',
        'db_pull_done': 'Finished pulling cloud records.\nPulled: {0} / skipped (existing): {1} / failed: {2}',
        'db_pull_cancelled': 'Pull cancelled.',
        'db_pull_failed': 'Failed to pull cloud records:\n{0}',
        'db_menu_logs': 'Fetch job logs',
        'db_logs_title': 'Training pipeline logs',
        'db_logs_select_run': 'Target run:',
        'db_logs_fetch': 'Fetch logs',
        'db_logs_fetching': 'Fetching logs...',
        'db_logs_fetch_failed': 'Failed to fetch logs: {0}',
        'db_logs_save': 'Save to file',
        'db_logs_saved': 'Logs saved:\n{0}',
        'db_logs_save_failed': 'Failed to save logs: {0}',
    },
}
